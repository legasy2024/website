...
import { blogPostsRegistry, getPostById, getPostBySlug } from "@/content/blog/posts-registry";
...
export async function getBlogPostById(id, locale) {
  ...
  try {
    ...
    if (!content.translations || !content.translations[locale]) {
      throw new Error(`Missing translations for locale "${locale}" in post "${id}"`);
    }
  ...
  return {
    ...
    image: t.image || content.image || postMetadata.image,
